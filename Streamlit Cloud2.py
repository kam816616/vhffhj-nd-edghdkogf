...
def get_fresh_proxy():
    """Get a fresh proxy from free proxy service"""
    try:
...
def get_proxy_session():
    """Create a session with rotating proxies and advanced fingerprinting"""
    session = requests.Session()
    
    # Get a fresh proxy (50% chance free proxy, 50% chance from our list)
    if random.choice([True, False]):
        proxy = get_fresh_proxy()
    else:
        proxy = {
            'http': random.choice(st.secrets.get("PROXY_LIST", [])),
            'https': random.choice(st.secrets.get("PROXY_LIST", []))
        }
...
def renew_tor_ip():
    """Renew Tor IP by requesting new circuit"""
    try:
        with Controller.from_port(port=9051) as controller:
            controller.authenticate(password=st.secrets.get("TOR_PASSWORD", ""))
            controller.signal(Signal.NEWNYM)
            time.sleep(controller.get_newnym_wait())
        return True
    except Exception as e:
        message_queue.put(f"Tor IP renewal failed: {str(e)}")
        return False
...
def renew_ip():
    """Force a new IP with advanced rotation strategy"""
    global session
    ...
    try:
        ...
        # Rotation strategy: 40% proxy, 40% Tor (if enabled), 20% fresh proxy
        choice = random.random()
        
        if choice < 0.4:
            # Regular proxy rotation
            session = get_proxy_session()
        elif choice < 0.8 and st.secrets.get("TOR_PASSWORD"):
            # Tor rotation
            if renew_tor_ip():
                session.proxies = {
...
def main():
    ...
    with st.sidebar:
        ...
        st.checkbox("Use Tor Network", 
                   value=False, 
                   key='use_tor',
                   help="Use Tor for additional anonymity (requires Tor running)")
