...
NUM_VISITS = int(st.secrets.get("NUM_VISITS", 20))
MIN_DELAY = int(st.secrets.get("MIN_DELAY", 5))
MAX_DELAY = int(st.secrets.get("MAX_DELAY", 15))
ENABLE_BROWSER = True
MAX_LOG_LINES = 1000  # Limit log size for Streamlit Cloud memory
...
def get_timezone_for_ip(ip_info):
    ...
    if ip_info and 'timezone' in ip_info:
        try:
            return pytz.timezone(ip_info['timezone']).zone
...
def get_random_device():
    ...
    try:
        ...
        message_queue.put(f"⚠️ Device generation error: {str(e)}")
        # Return a default device if error occurs
        return {
            "platform": "Windows",
            "model": {"name": "Desktop PC", "code": "Windows NT 10.0"},
...
def generate_platform_user_agent(device):
    ...
    try:
        ...
        message_queue.put(f"⚠️ User agent generation error: {str(e)}")
        return ua.random
...
def get_session():
    ...
    try:
        ...
        message_queue.put(f"⚠️ Session creation error: {str(e)}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('status') == 'success':
                return data
        message_queue.put(f"⚠️ IP API returned status: {response.status_code}")
        return None
    except Exception as e:
        message_queue.put(f"⚠️ IP info lookup failed: {str(e)}")
        return None
...
def renew_ip():
    ...
    try:
        ...
        message_queue.put("⚠️ IP did not change after multiple attempts")
        return None
    except Exception as e:
        message_queue.put(f"⚠️ IP renewal failed: {str(e)}")
        return None
...
def generate_advanced_fingerprint(device, ip_info):
    ...
    try:
        ...
        message_queue.put(f"⚠️ Fingerprint generation error: {str(e)}")
        return {
            "webgl": {},
            "canvas": {},
            "audio": {},
...
def get_random_headers(device, fingerprint):
    ...
    try:
        ...
        message_queue.put(f"⚠️ Header generation error: {str(e)}")
        return {
            'User-Agent': ua.random,
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
...
def simulate_google_search(search_query, device, fingerprint):
    ...
    try:
        ...
        if response.status_code != 200:
            message_queue.put(f"⚠️ Search failed with status {response.status_code}")
            return None
        ...
        if not results:
            message_queue.put("⚠️ No search results found")
            return None
        ...
        message_queue.put(f"⚠️ Search simulation failed: {str(e)}")
        return None
...
def simulate_real_visit(url, device, fingerprint):
    ...
    try:
        ...
        if response.status_code != 200:
            message_queue.put(f"⚠️ Page load failed with status {response.status_code}")
            return False
        ...
        if random.random() > 0.3:  # 70% chance of interaction
            ...
            if links:
                ...
                for _ in range(clicks):
                    ...
                    message_queue.put(f"🖱️ Clicking on: {link}")
                    session.get(link, headers=headers, timeout=15)
        
        return True
    except Exception as e:
        message_queue.put(f"⚠️ Visit simulation failed: {str(e)}")
        return False
            ...
            with col1:
                ...
                <div style="border:1px solid #ddd; border-radius:5px; padding:10px; height:500px; background-color:#f9f9f9;">
                    <div style="border-bottom:1px solid #ddd; padding-bottom:5px; margin-bottom:10px;">
                        ...
                        <span style="color:#666;">Anti-Detect Browser Simulation - {WEBSITE_URL}</span>
                    </div>
                    <div style="padding:10px; height:90%; overflow:auto; background-color:white;">
                        {st.session_state.get('browser_content', 'Waiting for simulation to start...')}
                    </div>
                </div>
...
def run_simulation():
    ...
    try:
        with simulation_lock:
            ...
            message_queue.put(f"🔢 Number of visits: {st.session_state.num_visits}")
            message_queue.put(f"⏱️ Delay range: {st.session_state.min_delay}-{st.session_state.max_delay} seconds")
            ...
            for i in range(st.session_state.num_visits):
                ...
                try:
                    ...
                    if i < st.session_state.num_visits - 1 and simulation_running:
                        ...
                        base_delay = (st.session_state.max_delay - st.session_state.min_delay) / 2
                        delay = max(st.session_state.min_delay, 
                                   min(st.session_state.max_delay, 
                                       random.gammavariate(2, 0.5) * base_delay + st.session_state.min_delay))
                        message_queue.put(f"⏳ Waiting {delay:.1f} seconds before next visit...")
                        time.sleep(delay)
                
                except Exception as e:
                    message_queue.put(f"⚠️ Visit {i+1} failed: {str(e)}")
        ...
        message_queue.put(f"❌ Critical simulation error: {str(e)}")
    finally:
        simulation_running = False
...
def main():
    st.set_page_config(
        page_title="Advanced Anti-Detect Simulator", 
        layout="wide",
        page_icon="🕵️‍♂️"
    )
    ...
    if 'simulation_running' not in st.session_state:
        ...
        st.session_state.num_visits = NUM_VISITS
    ...
    with st.sidebar:
        ...
        st.session_state.num_visits = st.number_input(
            "Number of Visits", 
            min_value=1, 
            max_value=100, 
            value=NUM_VISITS,
            help="Higher numbers may hit API rate limits"
        )
        col1, col2 = st.columns(2)
        with col1:
            st.session_state.min_delay = st.number_input(
                "Min Delay (s)", 
                min_value=1, 
                max_value=60, 
                value=MIN_DELAY,
                help="Minimum delay between visits"
            )
        with col2:
            st.session_state.max_delay = st.number_input(
                "Max Delay (s)", 
                min_value=1, 
                max_value=60, 
                value=MAX_DELAY,
                help="Maximum delay between visits"
            )
        ...
        with col1:
            if st.button(
                ...
                with st.session_state.lock:
                    ...
                    st.session_state.log_content = ""  # Clear previous logs
        ...
        st.info("ℹ️ Running on Streamlit Cloud - simulations auto-stop after 4 hours")
    ...
    with tab3:
        ...
        # Display only the last MAX_LOG_LINES to prevent memory issues
        log_placeholder.code(st.session_state.log_content[-MAX_LOG_LINES:])
        
        if st.button("Clear Logs", key="clear_logs"):
            st.session_state.log_content = ""
            log_placeholder.code("")
    
    # Update UI in a try-except block to prevent crashes
    try:
        while True:
            ...
            if new_messages:
                ...
                if len(st.session_state.log_content) > MAX_LOG_LINES * 2:
                    st.session_state.log_content = st.session_state.log_content[-MAX_LOG_LINES:]
                log_placeholder.code(st.session_state.log_content[-MAX_LOG_LINES:])
            
            # Update progress
            if st.session_state.simulation_running and st.session_state.num_visits > 0:
